...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.saleHistoryService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/sale-history'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: SaleHistory) {
        return item.id;
    }
    registerChangeInSaleHistories() {
        this.eventSubscriber = this.eventManager.subscribe('saleHistoryListModification', (response) => this.loadAll());
    }
