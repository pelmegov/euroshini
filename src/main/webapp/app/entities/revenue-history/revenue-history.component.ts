...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.revenueHistoryService.search({
                page: this.page - 1,
        ...
        this.revenueHistoryService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/revenue-history'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/revenue-history', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RevenueHistory) {
        return item.id;
    }
    registerChangeInRevenueHistories() {
        this.eventSubscriber = this.eventManager.subscribe('revenueHistoryListModification', (response) => this.loadAll());
    }
